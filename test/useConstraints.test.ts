...
describe('useConstraints', () => {
  ...
  it('keeps values in sync with sync validator', async () => {
    ...
    await validate({ start: 55, end: 75 }, { start: SUCCESS, end: SUCCESS });
    await validate({ start: 75, end: 75 }, { start: SUCCESS, end: SUCCESS });
    await validate({ start: 75.1, end: 75 }, { start: ERROR, end: ERROR });
  });
});
